...
const DEFAULT_TEXT_INPUT_PASSWORD_TOGGLE_CLASSNAME =
	'_text-input-password-toggle relative z-[10] flex h-6 w-6 mr-2 shrink-0 items-center justify-center rounded-full ring-offset-mode-accent p-0 [&]:data-[focus-visible=true]:ring-[2px]'

const DEFAULT_TEXT_INPUT_BASE_PASSWORD_TOGGLE_ICON_CLASSNAME =
	'_text-input-password-toggle-icon pointer-events-none absolute inset-0 left-1/2 top-1/2 z-[1] -translate-x-1/2 -translate-y-1/2 text-mode-contrast-accent/90 transition-[opacit,color] duration-fast'

const DEFAULT_TEXT_INPUT_CLEAR_BUTTON_CLASSNAME =
	'_text-input-clear-button h-6 w-6 shrink-0 rounded-full bg-mode-accent/50 p-0 [&]:data-[focus-visible=true]:ring-[2px]'

const DEFAULT_TEXT_INPUT_CLEAR_BUTTON_ICON_CLASSNAME =
	'_text-input-clear-button-icon text-mode-contrast-accent/90 transition-[color] duration-fast'
